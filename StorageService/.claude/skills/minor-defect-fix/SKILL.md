...
Работа разделена между **главным агентом** и **четырьмя типами субагентов**. Это даёт
три вещи:

1. Контекст главного агента не засоряется длинными выводами `gradle test`, дампами
   покрытия и тяжёлыми чтениями документации/спеки.
2. Каждый субагент стартует с чистым промптом, заточенным под одну задачу, и работает
   быстрее и точнее.
3. Главный агент сохраняет контроль над всем, что необратимо: правки кода, коммит,
   push, общение с Jira.

| Этап | Кто исполняет | Тип субагента |
|---|---|---|
| Конфиг, чтение Jira | главный агент | — |
| Скоуп-чек | главный агент | — |
| **Анализ задачи и кода** | **аналитик** | Explore |
| План фикса | главный агент | — |
| Правка кода | главный агент | — |
| **Написание/актуализация тестов** | **тестописатель** | general-purpose |
| **Прогон тестов и покрытия** | **тестраннер** | general-purpose |
| Pre-commit (build + lint + coverage) | тестраннер | general-purpose |
| **Обновление спецификации (отдельное репо)** | **спецадаптер** | general-purpose |
...
**Важно:** субагентов вызывай в момент, когда они нужны, а не пакетом наперёд. Каждый
вызов передаёт ровно тот контекст, который нужен для его задачи. Шаблоны промптов —
...
## 4. Анализ — субагент-аналитик

Делегируй разбор задачи и поиск места правки **Explore-субагенту**. Это read-only
агент: он не может ничего изменить, только читать и искать.

```
description: "Locate minor defect for <JIRA-KEY>"
subagent_type: Explore

prompt:
Найди в проекте место для минорной правки по задаче Jira.

Контекст задачи:
- Ключ: <JIRA-KEY>
- Summary: <summary>
- Описание: <description>
- Последние комментарии: <последние 3-5 комментариев>
- Components/labels: <если есть>

Корень репозитория кода: <git toplevel>
Репо со спецификацией (.md): <docs_path>

Что нужно вернуть (под 300 слов):
1. **Что сломано** — одно-два предложения своими словами.
2. **Где именно** — конкретные файлы и номера строк (`path/Foo.java:42`).
3. **Почему так происходит** — root cause, не симптом.
4. **Какие тесты затрагивает** — пути к тестам, которые покрывают этот код сейчас.
5. **Какие разделы спеки могут быть затронуты** — пути к .md в репо спеки, без правок.
6. **Edge cases** — что ещё может сломаться от наивной правки.
7. **Открытые вопросы** — что в задаче неясно и стоит уточнить у пользователя ДО фикса.

Не предлагай конкретный код фикса. Не редактируй файлы. Ищи прицельно: грепай по
именам классов/методов из описания, по текстам ошибок из логов/комментариев. В .md
ходи только по конкретному поводу, не читай целиком.
```

После получения ответа:
- Если у субагента есть "Открытые вопросы" — задай их пользователю **одним
  сообщением**.
- Если всё ясно — переходи к плану.
...
## 6. Реализация фикса — главный агент

Главный агент сам правит код. Это сознательно: правки кода — это точка, где должен
быть прямой контроль и видимый diff пользователю.

- Минимальное изменение, решающее задачу. Не подметай рядом.
- Не вводи новых абстракций, фича-флагов, "на будущее".
- Не добавляй комментариев "что" — только нетривиальное "почему".
- Не ломай публичные сигнатуры без явного согласования.

Если в проекте есть скилл `java-spring-dev` — следуй его конвенциям автоматически.
